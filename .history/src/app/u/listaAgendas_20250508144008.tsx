...
const TelaSelecaoAgendas = () => {
  ...
  const onConfirmar = (e: any) => {
    const listaselecao = e; // deve ser array de ids
  
    const novasAgendas = agendasDisponiveis.map((agenda) => {
      
      if (agenda.id != null && listaselecao(Number).includes(<Numb></Numb>agenda.id)) {
        console.log('khkhdkjf');
        
        agenda.setVisivel(true);
      }
      
      return agenda;
    });
  
    setAgendaDisponivel(novasAgendas); // só se for useState
  
    console.log(agendasDisponiveis); // agora vai mostrar visivel: true nos selecionados
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={agendasDisponiveis.map(agenda => ({ id: agenda.id ? String(agenda.id) : 'null', nome: agenda.informations.title || 'Sem Nome' }))}
