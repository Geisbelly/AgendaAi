...
import { ContratoExecucaoApi } from '@/backend/services/builder/apiContratoExecucao';
...
export const SessionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const checkSession = async () => {
      const token = await AuthService.getDadosSession();
      if (token) {
        const isValid = await SessionService.isTokenValid();
  ...
  const login = async (tokens: string) => {
    try {
      await AuthService.login(tokens);
      await SessionService.setToken(tokens)
