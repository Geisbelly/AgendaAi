...
import { EventInformations, Evento } from './models'; // Supondo que os modelos estejam em um arquivo separado
...
export function useEventos() {
    ...
    try {
      ...
      const result = await db.runAsync(
        ...
        [
          ...
          data.calendarTitle,
          data.created_at?.toISOString(), // Armazenando como string de data
          data.modified_at?.toISOString(), // Armazenando como string de data
        ]
      );
  ...
  async function getAll() {
    try {
      const db = await getDatabase();
      const result = await db.getAsync('SELECT * FROM evento');
