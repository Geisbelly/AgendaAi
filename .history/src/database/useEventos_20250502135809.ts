...
export function useEventos() {
  ...
  async function getAll() {
    try {
      const db = await getDatabase();
      const result: Evento[] = await db.getAllAsync('SELECT * FROM evento');
      if (result.length > 0) {
        const eventos = [];
        for (let i = 0; i < result.length; i++) {
          const row:Evento = result[i];
          eventos.push({
            ...row,
            informations: row.informations, // Já é do tipo EventInformations
            created_at: row.created_at ? new Date(row.created_at) : null,
            modified_at: row.modified_at ? new Date(row.modified_at) : null,
          });
        }
        return eventos;
      }
